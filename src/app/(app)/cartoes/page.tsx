...
export default async function CardsPage() {
  ...
  return <div className="space-y-7"><PageHeader title="Cartões" description="Limite e faturas conectados ao mesmo livro financeiro." />
    <Card><h2 className="mb-4 font-semibold">Novo cartão</h2><CardForm institutions={options.institutions.map((item) => ({ id: item.id, name: item.name }))} accounts={options.accounts.map((item) => ({ id: item.id, name: item.name, type: item.type }))} /></Card>
    {options.cards.length ? <Card><h2 className="mb-4 font-semibold">Compra parcelada</h2><InstallmentForm cards={options.cards.map((item) => ({ id: item.id, name: item.account.name }))} categories={options.categories.map((item) => ({ id: item.id, name: item.name }))} /></Card> : null}
