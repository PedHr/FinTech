...
export function CardForm({ institutions, accounts }: { institutions: Option[]; accounts: Option[] }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = event.currentTarget; setPending(true); const result = await createCardAction(new FormData(form)); setPending(false);
    if (!result.ok) return toast.error(result.error.message); form.reset(); toast.success("Cartão adicionado.");
  }
...
export function InstallmentForm({ cards, categories }: { cards: Option[]; categories: Option[] }) {
  const [pending, setPending] = useState(false);
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = event.currentTarget; setPending(true); const result = await createInstallmentAction(new FormData(form)); setPending(false);
    if (!result.ok) return toast.error(result.error.message); form.reset(); toast.success("Parcelas criadas nas faturas futuras.");
  }
